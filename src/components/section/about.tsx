...
const About = () => {
  return (
    <section id="about" className="mx-5 flex flex-col items-center justify-center space-y-11 py-20">
      ...
      <p className="text-fg flex flex-col items-center text-center text-sm leading-relaxed sm:text-base">
        <span>안녕하세요, 프론트엔드 개발자 정다빈입니다.</span>
        <span>문제를 해결할 때 몰입해 깊이 탐구하는 것을 즐기며, 맡은 일은 끝까지 책임집니다.</span>
        <span>열린 소통으로 팀워크를 강화하고 협업 효율을 높입니다.</span>
      </p>

      <div className="border-primary bg-primary/8 rounded-xl border p-4 sm:p-8">
        <ul className="grid grid-cols-2 gap-4 sm:grid-cols-2">
          {INFO.map((item) => (
            <li key={item.label} className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <item.icon className="text-primary w-4" />
